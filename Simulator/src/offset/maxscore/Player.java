...
public class Player extends offset.sim.Player {
  ...
  private double getStealScore(Move move, int id) {
    Point src = move.src;
    Point target = move.target;
    if (src.value == 1)
      return 1; //making this a little less attractive
    if (src.owner != self && target.owner != self)
      return 2*src.value; //we gain both the piles
    else if (src.owner != self || target.owner != self)
      return 1.9*src.value; //empirically
    else
      return 1.7*src.value; //empirically
  }
  ...
  private Move[] getBestStealMoves(Move[] allMoves, int id) {
    ...
    for (Move move : allMoves) {
      double tempScore = getStealScore(move,id);
  ...
  Move bestMove(Point[][] grid, int depth, Pair myPair, Pair herPair, double mybest, double herbest){
    ...
    //Ran over various d values and the average ticks I got was around 660, hence
    //trying out this beginning phase of focusing on stealing (1/3rd ticks of 660 = 220)and then moving on to 
    //focusing on cutting of moves
    if (tick < 220)
    {
    ...
    {
        moves = getCutoffMoves(moves, 10, grid, herPair);
        moves = getBestStealMoves(moves,minimaxID);
    }
